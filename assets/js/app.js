...
function updateToolTip(initialXAxis, circlesGroup) {
    ...
    var toolTip = d3.tip()
        ...
        .offset([80, -60])
        .html(function (d) {
            return (`${d.state}<br>In Labor Force: ${d.labor_force}% <br>With Depression: ${d.depression}%`)
        });
...
d3.csv("data.csv", function(error, healthData) {
        ...
        // create scale functions 
    var xLinearScale = d3.scaleLinear()
        .domain([d3.min(healthData, d => d.labor_force)-5, d3.max(healthData, d => d.labor_force)+5])
        .range([0, width])
